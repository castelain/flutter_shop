...
async function resolveSrcImport(root, block, ctx, resolver) {
    ...
    const importee = utils_1.cleanUrl(serverPluginModuleRewrite_1.resolveImport(process.cwd(), importer, block.src, resolver));
...
async function parseSFC(root, filePath, content) {
    ...
    if (errors.length) {
        ...
        errors.forEach((e) => {
            console.error(chalk_1.default.underline(`${filePath}:${e.loc.start.line}:${e.loc.start.column}`));
